...
CREATE TYPE "public"."skill_category" AS ENUM('timing', 'footwork', 'connection', 'musicality', 'styling', 'partnering', 'body_mechanics', 'patterns', 'improvisation', 'performance', 'custom');--> statement-breakpoint
...
ALTER TABLE "skills" ADD CONSTRAINT "skills_coach_student_id_coach_students_id_fk" FOREIGN KEY ("coach_student_id") REFERENCES "public"."coach_students"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "idx_skills_coach_student" ON "skills" USING btree ("coach_student_id");--> statement-breakpoint
CREATE INDEX "idx_skills_priority" ON "skills" USING btree ("coach_student_id","priority_score" DESC NULLS LAST);--> statement-breakpoint
