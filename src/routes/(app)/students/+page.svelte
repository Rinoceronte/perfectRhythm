...
<svelte:head>
	<title>Students — {data.branding.siteName}</title>
</svelte:head>
...
<div class="mx-auto max-w-3xl space-y-6 px-4 py-8">
				...
				<div class="py-3 text-center">
					...
					{#if addResult.studentCreated}
						<p class="mt-1 text-sm text-zinc-500">
							New account created — they'll need to set a password to log in
						</p>
					{/if}
				</div>
			{:else}
				...
				<div class="relative">
					...
					{#if emailMatch}
						<button
							onclick={clearMatch}
							class="absolute top-7 right-2 text-xs text-zinc-400 hover:text-zinc-600">clear</button
						>
					{/if}
	...
	<div class="flex gap-1 rounded-xl bg-zinc-100 p-1 text-sm">
		{#each [{ id: 'active', label: `Active (${activeCount})` }, { id: 'inactive', label: `Inactive (${inactiveCount})` }, { id: 'all', label: `All (${data.students.length})` }] as tab (tab.id)}
			<button
	...
	{#if data.students.length > 5}
		<input
			...
			class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-500 focus:ring-1 focus:ring-zinc-500 focus:outline-none"
		/>
	{/if}

	{#if filtered().length > 0}
		<ul class="divide-y divide-zinc-100 rounded-lg border border-zinc-200 bg-white">
			{#each filtered() as student (student.coachStudentId)}
				<li>
					<a
						href={resolve('/(app)/students/[id]', { id: student.studentId })}
						class="flex items-center gap-4 px-4 py-4 transition-colors hover:bg-zinc-50"
					>
						{#if student.avatarUrl}
							<img src={student.avatarUrl} alt="" class="h-10 w-10 rounded-full object-cover" />
						{:else}
							<div
						...
						<div class="min-w-0 flex-1">
							<div class="flex items-center gap-2">
								...
								{#if !student.isActive}
									<span class="rounded-full bg-zinc-100 px-2 py-0.5 text-xs text-zinc-500"
										>Inactive</span
									>
								{/if}
							</div>
							<p class="text-sm text-zinc-500">
								{#if student.nextLessonAt}
									Next lesson {formatDistanceToNow(new Date(student.nextLessonAt), {
										addSuffix: true
									})}
								{:else}
									No upcoming lessons
	...
	{:else if filter === 'active'}
		<div class="rounded-lg border border-dashed border-zinc-300 py-12 text-center">
			...
			<p class="mt-1 text-xs text-zinc-400">
				Students become active after a confirmed lesson in the last 6 months.
			</p>
		</div>
