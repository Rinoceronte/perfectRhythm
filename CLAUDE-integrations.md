...
const SMS_TEMPLATES: Partial<Record<NotificationType, (data: Record<string, string | number>) => string>> = {
  video_review_ready: (d) => `Your video review from ${d.coachName} is ready. View it in the app.`,
  lesson_booking_confirmed: (d) => `Lesson confirmed with ${d.coachName} on ${d.date} at ${d.time}.`,
  booking_window_open: (d) => `Booking is now open for ${d.eventName}. Open the app to book.`,
  priority_booking_window_open: (d) => `Early access: booking for ${d.eventName} is open for you. Book now before general release.`,
...
export async function geocodePlace(placeId: string): Promise<{ lat: number; lng: number; formattedAddress: string }> {
